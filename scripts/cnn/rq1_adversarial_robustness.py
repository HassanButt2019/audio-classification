"""
RQ1 — Adversarial Robustness of CNN Training Strategies.
...
Outputs saved to:
  research_answers/cnn/AtTraining_RQ1/

Usage
-----
  python scripts/cnn/rq1_adversarial_robustness.py
"""
...
EPS_LABELS = ["eps=0.01", "eps=0.03", "eps=0.10"]   # display labels


# ── helpers ────────────────────────────────────────────────────────────────────

def _find_cv_summary(mode):
    base = os.path.join(ROOT, "results", "cnn", mode)
...
def _fold_mean(attack_dict, eps_key):
    """Average a per-fold attack dict over all 10 folds for one epsilon key."""
    vals = [
...
def load_strategy_stats():
    """Return a list of dicts with aggregated stats per strategy."""
    stats = []
    for mode, strategy in MODES:
        ...
        # clean accuracy: mean across per_fold entries
...
def write_summary(stats, out_dir):
    """Table 1 — clean + FGSM + BIM accuracy per strategy."""
    headers = (
        ["Strategy", "Clean (%)"]
...
def write_drop(stats, out_dir):
    """Table 2 — accuracy drop (percentage points) from clean baseline."""
    headers = (
        ["Strategy", "Clean (%)"]
    ...
    for s in stats:
        ...
        for eps in EPSILONS:
            drop = s["clean_acc"] - s["fgsm"][eps]
            row.append(f"{drop:.2f}")
        for eps in EPSILONS:
            drop = s["clean_acc"] - s["bim"][eps]
            row.append(f"{drop:.2f}")
        rows.append(row)
...
def write_cross_attack(stats, out_dir):
    """Table 3 — cross-attack robustness.

    For each strategy shows:
      - Mean FGSM robustness (avg across all 3 epsilons)
      - Mean BIM  robustness (avg across all 3 epsilons)
      - FGSM-BIM gap (positive = better under FGSM than BIM)

    This reveals transfer: does AT-FGSM generalise to BIM attacks?
    Does AT-BIM generalise to FGSM attacks?
    """
    headers = [
        "Strategy",
        "Mean FGSM Acc (%)",
...
def write_json(stats, cross_json, out_dir):
    """Full results as JSON for programmatic use."""
    payload = []
    for s in stats:
        payload.append({
            "strategy":  s["strategy"],
            ...
            "fgsm": {
                f"eps_{eps}": round(s["fgsm"][eps], 4) for eps in EPSILONS
            },
            "bim": {
                f"eps_{eps}": round(s["bim"][eps], 4) for eps in EPSILONS
            },
            "fgsm_drop": {
...
def main():
    print("=" * 60)
    print("RQ1 — CNN Adversarial Robustness Analysis")
    print("Strategies: Baseline | AT-FGSM | AT-BIM | AFT-FGSM | AFT-BIM")
    print("Attacks:    FGSM + BIM  |  eps = 0.01, 0.03, 0.10")
    print("=" * 60)

    out_dir = os.path.join(ROOT, "research_answers", "cnn", "AtTraining_RQ1")
