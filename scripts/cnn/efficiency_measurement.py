"""
Efficiency measurement for CNN — RQ2 (Inference) and RQ3 (Training).

Outputs
-------
ptflops/cnn/Inference Efficiency (RQ2)/rq2_inference_efficiency.csv
ptflops/cnn/Inference Efficiency (RQ2)/rq2_inference_efficiency.json
ptflops/cnn/Training Efficiency (RQ3)/rq3_training_efficiency.csv
ptflops/cnn/Training Efficiency (RQ3)/rq3_training_efficiency.json

Usage
-----
  python scripts/cnn/efficiency_measurement.py
"""
...
# CNN input: (channels=1, mel_bins=64, time_frames=128)
INPUT_RES = (1, 64, 128)
...
def _find_cv_summary(mode):
    """Return path to cv_summary.json for a given mode.

    Handles both a direct path and a date-stamped subdirectory (used for the
    normal mode where the run ID is embedded in the folder name).
    """
...
def _measure_flops():
    """Run ptflops on a fresh CNN instance (architecture-level measurement)."""
    model = UrbanSoundCNN()
    model.eval()
...
def main():
    print("=" * 60)
    print("CNN Efficiency Measurement")
    ...
    rq2_dir = os.path.join(ROOT, "ptflops", "cnn", "Inference Efficiency (RQ2)")
    ...
    rq3_dir = os.path.join(ROOT, "ptflops", "cnn", "Training Efficiency (RQ3)")
